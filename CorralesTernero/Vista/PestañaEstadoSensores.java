...
public class PestañaEstadoSensores extends JPanel {
	...
	private void hazInterfaz() {
		...
		btnEnviarCorreo.setBounds(370, 10, 80, 80);
	...
	public void setTabla(Vector<Vector<Object>> objetoCria) {
		...
		for (int i = 0; i < objetoCria.size(); i++) {
			...
			nuevoSensor.add(objetoCria.get(i).get(1)); // ID CRIA
			nuevoSensor.add(objetoCria.get(i).get(2) + " ºC"); // Temperatura
			nuevoSensor.add(objetoCria.get(i).get(3) + " mm Hg"); // Presión arterial
			nuevoSensor.add(objetoCria.get(i).get(4)); // Ubicación
